...
vector<double> LatticeBoltzmann::Derivatives(int ix, int iy, double dt)
{
  ...
  { // on each direction
    ...
    rhoNext = rho(ixnext, iynext, false);
    UxNext = Jx(ixnext, iynext, false) / rhoNext;
    UyNext = Jy(ixnext, iynext, false) / rhoNext; // preguntarse si usar false o true: false si se usa luego de advección.
